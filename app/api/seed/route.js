...
export async function GET(request) {
  ...
  if (action === 'init') {
    ...
    if (existing.length > 0) return NextResponse.json({ message: 'Race already exists', race: existing[0] })
  ...
  if (action === 'add') {
    ...
    if (token.length === 0) {
      ...
      token = await sql`INSERT INTO tokens (contract_address, name, ticker, logo_url, current_mcap, status, votes) VALUES (${ca}, ${info.name}, ${info.ticker}, ${info.logo_url}, ${info.current_mcap}, 'queued', 50) RETURNING *`
    } else {
  ...
  if (action === 'start') {
    ...
    for (const token of queued) {
      ...
      await sql`INSERT INTO race_entries (race_id, token_id, start_mcap, current_mcap, peak_mcap) VALUES (${race[0].id}, ${token.id}, ${mcap}, ${mcap}, ${mcap}) ON CONFLICT (race_id, token_id) DO NOTHING`
  ...
  return NextResponse.json({ error: 'Unknown action', usage: { init: '/api/seed?secret=X&action=init', add: '/api/seed?secret=X&action=add&ca=CA', start: '/api/seed?secret=X&action=start', status: '/api/seed?secret=X&action=status' } }, { status: 400 })
}
