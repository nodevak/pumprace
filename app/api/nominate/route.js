...
export async function POST(request) {
  try {
    ...
    if (!info) return NextResponse.json({ error: 'Token not found on Solana DexScreener.' }, { status: 404 })
    if (info.current_mcap < 1000) return NextResponse.json({ error: 'Token market cap is too low.' }, { status: 400 })
...
export async function GET() {
  try {
    ...
    const tokens = await sql`
      ...
      FROM tokens WHERE status IN ('pending', 'queued')
